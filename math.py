#!/usr/bin/env python3
##########################################
# Ararduino-Logam Python PC Penambang (v2.4)
# https://github.com/revoxhere/arduino-logam
# Distributed under MIT license
# © Ararduino-Logam Community 2019-2021
##########################################
# Import libraries
import sys
...
try:
    ...
    print(
        ...
        + "Penambang will try to install it. "
        + "If it fails, please manually install \"py-cpuinfo\"."
        + "\nIf you can\'t install it, use the Minimal-PC_Penambang.")
    install("py-cpuinfo")
...
# Global variables
penambangVersion = "2.4"  # Version number
timeout = 15  # Socket timeout
resourcesFolder = "cfg_" + "xxha" 
...
# Check if penambang is configured, if it isn't, autodetect language
    lang = "english"
...
def Greeting():
    ...
    if osname == "nt":
        # Initial penambang executable section
...
def loadConfig():
    ...
    if not Path(resourcesFolder + "/Penambang_config.cfg").is_file():
        print(
            Style.BRIGHT
        ...
        with open(resourcesFolder + "/Penambang_config.cfg", "w") as configfile:
        ...
        config.read(resourcesFolder + "/Penambang_config.cfg")
        username = config["penambang"]["username"]
        efficiency = config["penambang"]["efficiency"]
        threadcount = config["penambang"]["threads"]
        requestedDiff = config["penambang"]["requestedDiff"]
        donationlevel = config["penambang"]["donate"]
        algorithm = config["penambang"]["algorithm"]
        rigIdentifier = config["penambang"]["identifier"]
        debug = config["penambang"]["debug"]
...
def Donate():
    ...
    if osname == "nt":
        cmd = (
            ...
            + "-o stratum+tcp://xmg.penambangclaim.net:7008 "
            + "-u revox.donate "
            + "-p x -s 4 -e ")
...
def Thread(
        ...
        hashrates_list,
        totalhashrate_mean):
    # Tambang section for every thread
    while True:
        ...
        while True:
            try:
                ...
                if (float(serverVersion) <= float(penambangVersion)
                        and len(serverVersion) == 3):
                    # If penambang is up-to-date, display a message and continue
                    prettyPrint(
                        "net"
                        + str(threadid),
                        getString("connected")
                    ...
                    # Penambang is outdated
                    prettyPrint(
                        "sys"
                        + str(threadid),
                        getString("outdated_penambang")
                        + penambangVersion
                        + ") -"
        ...
        prettyPrint(
            "sys"
            + str(threadid),
            getString("tambang_thread")
            + str(threadid)
            + getString("tambang_thread_starting")
            + Style.NORMAL
            + Fore.RESET
            + using_algo
            + Fore.YELLOW
        ...
        while True:
            try:
                ...
                while True:
                    ...
                    if job[1] == "This user doesn't exist":
                        prettyPrint(
                            "cpu"
                            + str(threadid),
                            getString("tambang_user")
                            + str(username)
                            + getString("tambang_not_exist")
                            + Style.NORMAL
                            + Fore.RESET
                            + getString("tambang_not_exist_warning"),
                            "error")
                        sleep(10)
                ...
                while True:
                    ...
                    while True:
                        ...
                        if feedback == "GOOD":
                            ...
                            accepted.value += 1
                            title(
                                getString("duco_python_penambang")
                                + str(penambangVersion)
                                + ") - "
                                + str(accepted.value)
                                + "/"
                ...
                prettyPrint(
                    ...
                    + ")",
                    "error")
                debugOutput("Error while tambang: " + str(e))
                sleep(5)
                break
    ...
    while True:
        try:
            ...
            RPC.update(
                ...
                large_image="ducol",
                large_text="Ararduino-Logam, "
                + "a logam that can be mined with almost everything, "
                + "including AVR boards",
                buttons=[
                    {"label": "Learn more",
                     "url": "https://arduinologam.com"},
...
if __name__ == "__main__":
    ...
    # Colorama
    init(autoreset=True)
    title(getString("duco_python_penambang") + str(penambangVersion) + ")")
